...
{
    public class TrainerLoginViewModel: ViewModelBase
    {
        private TrainingHelperWebAPIProxy proxy;
        private IServiceProvider serviceProvider;
        public TrainerLoginViewModel(TrainingHelperWebAPIProxy proxy, IServiceProvider serviceProvider)
        {
        ...
        private async void OnLogin()
        {
            ...
            Trainer? u = await this.proxy.TrainerLoginAsync(loginInfo);
            ((App)Application.Current).OwnerIn = false;//set owner user
            ((App)Application.Current).TraineeIn = false;//set trainee user
            ((App)Application.Current).TrainerIn = true;//set trainer user
            InServerCall = false;

            //Set the application logged in user to be whatever user returned (null or real user)
            ((App)Application.Current).LoggedInTrainer = u;
            if (u == null)
            {
